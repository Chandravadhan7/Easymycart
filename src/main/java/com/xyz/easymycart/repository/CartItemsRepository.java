...
public interface CartItemsRepository extends JpaRepository<CartItems, Long> {
  ...
  @Query(
      value =
          "select * from cart_items ct where ct.cart_id = :cartId and ct.product_id = :productId",
      nativeQuery = true)
  ...
  @Query(
      value =
          "update cart_items ct set ct.quantity = ct.quantity + 1 where ct.product_id = :productId",
      nativeQuery = true)
  ...
  @Query(
      value =
          "update cart_items ct set ct.quantity = ct.quantity - 1 where ct.product_id = :productId and ct.quantity > 1",
      nativeQuery = true)
